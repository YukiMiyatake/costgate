...
function resolveServerWriteToken(options = {}) {
  if (options.writeToken != null && String(options.writeToken).length > 0) {
    return String(options.writeToken);
  }
  const env = process.env.COSTGATE_DASHBOARD_TOKEN ?? "";
  if (env) return env;
...
function createDashboardServer(options = {}) {
  const writeToken = resolveServerWriteToken(options);
  const tokenAutoGenerated =
    options.writeToken == null && !(process.env.COSTGATE_DASHBOARD_TOKEN ?? "");
...
async function main() {
  ...
  server.listen(PORT, HOST, () => {
    ...
    if (server.costgateWriteTokenAutoGenerated) {
