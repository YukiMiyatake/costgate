...
} from "./lib/dashboard-write-token.mjs";
...
function isWriteAuthorized(req, writeToken) {
  if (!writeToken) return false;
  const header = req.headers["x-costgate-dashboard-token"];
  return header === writeToken;
}

/** @deprecated prefer resolveDashboardWriteToken — kept for tests/export */
function resolveServerWriteToken(options = {}) {
  return resolveDashboardWriteToken(options).token;
}
...
async function handleWorkspaceRoute(method, pathname, url, req, res, ctx) {
  const { dataOptions, controlPaths, marketplaceDirPath, authorizeWrite } = ctx;
...
function createDashboardServer(options = {}) {
  const resolved = resolveDashboardWriteToken(options);
  const writeToken = resolved.token;
  const tokenAutoGenerated = resolved.source === "generated" || resolved.source === "file";
  const authorizeWrite = (req) => isWriteAuthorized(req, writeToken);
  ...
  const routeCtx = { dataOptions, controlPaths, marketplaceDirPath, authorizeWrite, writeToken };
    ...
    try {
      ...
      if (method === "GET") {
        if (pathname === "/api/health") {
          const health = {
            ...buildHealth({ writeTokenRequired: true }),
            ui: buildUiSettingsApiPayload(),
          };
          // Loopback-only: auto-fill the Dashboard write token so marketplace
          // Add MCP works when Gate spawned the server (token not in UI).
          if (resolved.bootstrappable && isLoopbackRequest(req)) {
            health.writes = {
              ...health.writes,
              bootstrap_token: writeToken,
              token_file: resolved.path,
            };
          }
          json(res, 200, health);
          return;
  ...
  server.costgateWriteToken = writeToken;
  server.costgateWriteTokenAutoGenerated = tokenAutoGenerated;
  server.costgateWriteTokenMeta = resolved;
  return server;
}
...
async function main() {
  ...
  const writeToken = server.costgateWriteToken;
  server.on("error", (err) => {
  ...
  server.listen(PORT, HOST, () => {
    ...
    console.log(`  http://${HOST}:${PORT}`);
    console.log(`  writes require X-Costgate-Dashboard-Token`);
    const meta = server.costgateWriteTokenMeta;
    if (meta?.path) {
      console.log(`  write token file: ${meta.path}`);
    } else if (meta?.source === "env") {
      console.log(`  token: COSTGATE_DASHBOARD_TOKEN`);
    } else {
      console.log(`  generated token (paste in Dashboard UI): ${writeToken}`);
    }
...
export { createDashboardServer, HOST, PORT, UI_DIR, normalizePathname, resolveServerWriteToken };
