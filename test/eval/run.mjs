...
import { mockGateEnv, baseGateEnv } from "../../scripts/lib/paths.mjs";
import { buildEvalReport, runEvalTask } from "../../scripts/lib/eval-harness.mjs";
    ...
    modes: Object.fromEntries(
      Object.entries(
        results.reduce((acc, r) => {
          if (!acc[r.mode]) acc[r.mode] = { passed: 0, failed: 0, total: 0, tasks: [] };
          acc[r.mode].total++;
          if (r.passed) acc[r.mode].passed++;
          else acc[r.mode].failed++;
          acc[r.mode].tasks.push(r);
          return acc;
        }, {})
...
async function main() {
  ...
  for (const modeKey of modeKeys) {
    ...
    for (const task of spec.tasks) {
      ...
      const result = await runEvalTask(task, env, {
        label: `eval-${modeKey}-${task.id}`,
        startupMs: liveMode ? 15000 : 4000,
      });
      results.push({ ...result, mode: modeKey });
    }
  }
