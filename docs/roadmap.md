...
| **28. Project-scoped dashboard** | 📋 Planned | Workspace registry, per-project MCP config |
| **29. Marketplace expansion** | 📋 Planned | 有名 MCP カタログ拡充、カテゴリ UI |
...
Phase 28     プロジェクト別設定   ← 主戦場
Phase 29     MCP マーケット拡充   ← 並行可
...
次の開発は **Phase 28 プロジェクトスコープ Dashboard**（本 repo）。costgate-cloud は **Phase 30+**（後回し）。
...
## Upcoming — OSS Dashboard (28)

Phase 23–27 でローカル Dashboard の可視化・制御・追加・推奨は完了。  
Phase 28 では **Cursor ワークスペース単位** で MCP 設定（Enable、Filesystem PATH 等）を管理する。
...
| **28. Project-scoped dashboard** | 📋 Planned | Activity Registry + ワークスペース別 read/write |
| **29. Marketplace expansion** | 📋 Planned | カタログ拡充 + カテゴリ UI + フィルタ |
...
| Plan | 現状 | 次の一手（優先順） |
|------|------|-------------------|
| **Free (OSS)** | Gate 削減 + Probe + CLI + eval + Dashboard（Phase 23–27 完了） | **Phase 28–29 Dashboard** |
| **Pro** | cloud MVP（手動 upload） | Phase 30+ ホスト Dashboard（OSS 23–25 拡張） |
| **Team** | — | Phase 32–33（後回し） |

Pro/Team の新機能開発は **OSS Phase 29 まで凍結**（Dashboard は OSS で先行）。既存 `cloud:upload` / Reporter はメンテのみ。
