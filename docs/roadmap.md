...
次の開発は **Phase 29 Marketplace expansion**（本 repo）。costgate-cloud は **Phase 30+**（後回し）。
...
## Upcoming — OSS Dashboard (29)

Phase 23–28 でローカル Dashboard の可視化・制御・追加・推奨・**ワークスペース別設定**は完了。  
Phase 29 では **MCP マーケット** のカタログ拡充とカテゴリ UI を行う。
...
| Plan | 現状 | 次の一手（優先順） |
|------|------|-------------------|
| **Free (OSS)** | Gate 削減 + Probe + CLI + eval + Dashboard（Phase 23–28 完了） | **Phase 29 Marketplace expansion** |
