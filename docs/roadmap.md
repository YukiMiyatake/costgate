...
| **6. costgate-cloud** | ✅ MVP | LoopGate metrics API, Reporter（**cloud repo のみ**） |
...
### Phase 6 — costgate-cloud ✅ (MVP · cloud repo only)
...
- **OSS からクラウドへ送信する機能は持たない**（`cloud:upload` 削除済み）
...
## Deferred — costgate-cloud（LoopGate · 製品）

**ステータス: 後回し。** 詳細は costgate-cloud `docs/product/roadmap.md`。

| Phase | 内容 | 備考 |
|-------|------|------|
| **LG-1+** | Hosted Runner, Gateway, Console | AWS Staging → Production |
| **LG-3** | **組織 / Team ポリシー** | OSS には置かない |
| **LG-5** | Enterprise BYOK / VPC / SIEM | |

**OSS との境界:** CostGate OSS は **完全ローカル**。クラウド API・upload・組織ポリシーは LoopGate のみ。
...
| **Pro** | cloud MVP | Phase LG-2+ LoopGate（**OSS からは独立**） |
| **Team** | — | LoopGate Pro（組織ポリシー — OSS 外） |

LoopGate 開発は costgate-cloud `docs/product/roadmap.md` 参照。**OSS はクラウド連携なし。**
