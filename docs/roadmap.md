...
| **2** | **OSS Dashboard**（Phase 23–28） | CLI 計測の UX 化・MCP ライフサイクル |
...
| Plan | 現状 | 次の一手（優先順） |
|------|------|-------------------|
| **Free (OSS)** | Gate 削減 + Probe + CLI + eval + Dashboard（Phase 23–27 完了） | **Phase 28 プロジェクト別 Dashboard** |
| **Pro** | cloud MVP（手動 upload） | Phase 30+ ホスト Dashboard（OSS 23–25 拡張） |
| **Team** | — | Phase 32–33（後回し） |

Pro/Team の新機能開発は **OSS Phase 28 まで凍結**（Dashboard は OSS で先行）。既存 `cloud:upload` / Reporter はメンテのみ。
