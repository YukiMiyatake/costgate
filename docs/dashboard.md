...
> **ステータス:** Phase 27 まで実装済み。Phase 28（プロジェクト別設定）・Phase 29（マーケット拡充）は計画中。  
...
| **MCP マーケット** | カテゴリ別一覧・有名 MCP カタログ | Phase 29 📋 |
...
### プロジェクト別設定（Phase 28・計画中）

現状の Dashboard は **グローバル設定**（`~/.costgate/`）のみです。Phase 28 では:

- Cursor で Gate を使った **ワークスペースごと** に MCP を管理
- Filesystem MCP の **ALLOWED_PATH もプロジェクト別**
- 一覧は Gate の **利用実績（Activity Registry）** から自動生成（ディスクスキャンなし）

詳細: [roadmap.md](./roadmap.md) Phase 28 / [dev/dashboard.md](./dev/dashboard.md)

### MCP マーケット拡充（Phase 29・計画中）

現状の Add MCP は **3 件のテンプレ** とキーワード検索のみです。Phase 29 では:

- **有名 MCP を大幅追加**（PostgreSQL, Playwright, Fetch, Linear, Docker 等 — [調査 doc](./mcp-reduction-survey.md) ベース）
- **機能別カテゴリ**（DevTools / Browser / Database / Search / SaaS / Cloud / AI）
- **カテゴリタブ・フィルタ・ソート**、公式 / Gate 対応バッジ、導入済み表示
