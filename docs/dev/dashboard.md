...
```jsonc
{
  ...
  "blind_spots": ["serena", "cursor-app-control"]
}
```
...
| 段階 | 実装 |
|------|------|
| 26a | `catalog/marketplace/*.json` 手動キュレーション |
| 26b | MCP Registry / Smithery API プロキシ（キャッシュ付き） |
...
| 既に Serena 有効 | `mcp.json` | code intelligence は直結維持を説明 |
...
costgate-cloud **Phase 30** は OSS Phase 23–25 の UI/API をホスト版に拡張（履歴・チーム・認証）。
...
- [ ] 既定 bind `127.0.0.1` のみ
- [ ] 書き込み API はトークン or CSRF 対策
- [ ] `mcp.json` 編集前に必ず backup
