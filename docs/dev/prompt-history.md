...
```
1. prompt-intent 履歴行 H を generation_id でインデックス
2. gate JSONL イベント E に generation_id があれば直接 JOIN
3. なければ:
     window_start = H.ts
     window_end   = min(次の H'.ts, H.ts + 5min)
     E.ts ∈ [window_start, window_end] かつ
     E.project_root ≈ H.workspace_root
4. ターン集計:
...
- Gate 再起動直後は `generation_id` 伝播前後で窓クラスタに落ちる可能性
