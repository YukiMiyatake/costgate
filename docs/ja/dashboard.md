...
> **言語:** [English](../dashboard.md) · 日本語（このファイル）
...
| **Prompt intent 表示** | Cursor Hook が推定した intent を Overview に表示 | Phase 28c ✅ |
| **プロンプト履歴** | 直近ターンの MCP トークン・tool_call・Export | Phase 9 ✅ |
| **Gate 設定** | compress / code-mode / intent 等を Dashboard から ON/OFF | Phase 30 ✅ |

---

## 起動方法

### 自動（推奨 — `@costgate/cli` / Gate MCP 起動時）

```bash
npx @costgate/cli@latest init   # 初回のみ
```

**costgate-gate** が Cursor から接続されると、Dashboard がバックグラウンドで起動し、初回のみブラウザが開きます。`costgate gate`（`npx @costgate/cli gate`）経由で `COSTGATE_DASHBOARD_AUTO=1` が有効です。

リポジトリ clone 利用時は `npm run cursor:production` でも同様です。
...
# または clone 後: npm run dashboard
# → http://127.0.0.1:8787
```

Gate を使わず Dashboard だけ試す場合や、ポート・トークンを指定したい場合は手動起動が便利です。
...
- 直近のプロンプトターン（既定 **50** 件）— `~/.costgate/history/turns.jsonl`
- ターンごとに preview / keywords、tools/list・tool_call 推定トークン、圧縮削減量
- Gate JSONL と `generation_id` で相関（なければ時間窓）
- 行を選択して **Export selected** → JSON ダウンロード

| 環境変数 | 既定 | 説明 |
|---------|------|------|
| `COSTGATE_HISTORY` | `1` | `beforeSubmitPrompt` でターン記録 |
| `COSTGATE_HISTORY_LIMIT` | `50` | ディスクに保持する最大件数 |
| `COSTGATE_HISTORY_PROMPT` | `preview` | `off` / `preview`（120 文字）/ `full` |

詳細: [dev/prompt-history.md](../dev/prompt-history.md)
...
| **Pro** | クラウド履歴・共有（LoopGate Console — costgate-cloud） |

個人ユーザーは **OSS ローカルダッシュボード** で可視化・最適化が完結。組織ポリシー・クラウド履歴は **LoopGate（有料）** — [ecosystem/plans.md](./ecosystem/plans.md)。
